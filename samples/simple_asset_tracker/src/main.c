...
/* Declare buffer to send GPS data in */
...
/*---------------------------------------------------------------------------*/
/**@brief Send GPS information to Datacake. */
static void gps_data_send(void)
{
	int err;

	/* Format and store latitude and longitude in gps_data buffer */
	err = snprintf(gps_data, MESSAGE_SIZE, "%.06f, %.06f", 
						current_pvt.latitude, current_pvt.longitude);

	if (err < 0) {
		LOG_ERR("Failed to encode gps data buffer: %d", err);
	}

	err = data_publish(&client,
					MQTT_QOS_1_AT_LEAST_ONCE,
					CONFIG_MQTT_PUB_TOPIC_LOCATION,
					gps_data,
					strlen(gps_data));
	if (err) {
...
void main(void)
{
	...
	LOG_INF("Stratus Simple Tracker started, version: %s", CONFIG_APP_VERSION);
