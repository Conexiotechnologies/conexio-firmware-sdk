...
static void cloud_update_work_fn(struct k_work *work)
{
	...
	/* Create a simulated temperature message to send to the cloud */
	...
	/* Construct a json message for the temp data */
	...
	if (msg.len < 0) {
		LOG_ERR("Failed to create temperature cloud message");
		return;
	}
...
void main(void)
{
	...
	LOG_INF("Conexio nRF Cloud client example has started");
