...
#error "No bosch_bme280 compatible node found in the device tree"
#endif
...
static void env_sensors_poll_fn(struct k_work *work)
{
	...
	if (IS_ENABLED(CONFIG_BME280)) {
		err = sensor_sample_fetch(bme280_dev);
		if (err) {
			LOG_ERR("Failed to fetch data from bme280 error: %d", err);
		}
	}

	for (int i = 0; i < num_sensors; i++) {
		if (!(IS_ENABLED(CONFIG_BME280))) {
			err = sensor_sample_fetch_chan(bme280_dev,
				env_sensors[i]->channel);
			if (err) {
				LOG_ERR("Failed to fetch data from bme280 error: %d",err);
			}
		}
		err = sensor_channel_get(bme280_dev,
