...
LOG_MODULE_REGISTER(watchdog, CONFIG_WDTIMER_LOG_LEVEL);
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	...
	LOG_INF("Watchdog timeout installed. Timeout: %d ms", CONFIG_WATCHDOG_TIMEOUT_MSEC);
	return 0;
}
...
static int watchdog_start(struct wdt_data_storage *data)
{
	...
	if (err) {
		...
		LOG_INF("Watchdog timer started");
	}
	return err;
}
...
static int watchdog_feed_enable(struct wdt_data_storage *data)
{
	...
	k_work_schedule(&data->system_workqueue_work,K_MSEC(WDT_FEED_WORKER_DELAY_MS));
	
	LOG_DBG("Watchdog feed enabled. Timeout: %d ms", WDT_FEED_WORKER_DELAY_MS);
